...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const responseSiswa = await fetch(`${BASE_URL}api/siswa`)
        .then((res) => res.json())
        .then((data) => (Array.isArray(data) ? data : [])); // Check if data is an array
      setJumlahSiswa(responseSiswa.length);

      const responsePengumuman = fetch(`${BASE_URL}api/pengumuman`)
        .then((res) => res.json())
        .then((data) => (Array.isArray(data.data) ? data.data : []));

      const responseTanyaJawab = fetch(`${BASE_URL}api/tanyajawab`)
        .then((res) => res.json())
        .then((data) => (Array.isArray(data) ? data : []));

      const responsePengajar = await fetch(`${BASE_URL}api/pengajar`)
        .then((res) => res.json())
        .then((data) => (Array.isArray(data) ? data : [])); // Ensure data is an array

      const responseAbsensi = fetch(`${BASE_URL}api/absensi`)
        .then((res) => res.json())
        .then((data) => (Array.isArray(data.data) ? data.data : []));

      const dataSiswa = (await responseSiswa) || [];
      const dataPengumuman = (await responsePengumuman) || [];
      const dataPengajar = responsePengajar || [];
      const dataTanyaJawab = (await responseTanyaJawab) || [];
      const absensiData = (await responseAbsensi) || [];

      const siswaLakiLaki = dataSiswa.filter(
        (siswa) => siswa?.Jenis_Kelamin === "Laki-Laki"
      );
      const siswaPerempuan = dataSiswa.filter(
        (siswa) => siswa?.Jenis_Kelamin === "Perempuan"
      );
      const pengajarLakiLaki = dataPengajar.filter(
        (pengajar) => pengajar?.Jenis_Kelamin === "Laki-Laki"
      );
      const pengajarPerempuan = dataPengajar.filter(
        (pengajar) => pengajar?.Jenis_Kelamin === "Perempuan"
      );
      ...
      setJumlahPengumumanHariIni(
        dataPengumuman.filter((pengumuman) => {
      ...
      setJumlahTanyaJawabHariIni(
        dataTanyaJawab.filter((tanyaJawab) => {
      ...
      const pengumumanHariIni = dataPengumuman.data
        ? dataPengumuman.data.filter((pengumuman) => {
            const createdDate = new Date(pengumuman?.Created_At);
            createdDate.setHours(0, 0, 0, 0);
            return createdDate.getTime() === today.getTime();
          })
                    ...
                    <div className="card-body">
                      <div className="d-flex">
                        <div className="flex-grow-1">
                          <p className="text-truncate font-size-18 mb-2">
                            Siswa
                          </p>
                          <h4 className="mb-2">
                            <label
                          ...
                          <p className="text-muted mb-0">
                            <span className="text-primary fw-bold font-size-12 me-2">
                              ...
                              {jumlahSiswaLakiLaki} Pria
                            </span>
                            <span
                              className="text-danger fw-bold font-size-12 me-2"
                              style={{ marginLeft: "10px" }}
                            >
                              <i className="ri-user-2-line me-1 align-middle"></i>
                              {jumlahSiswaPerempuan} Wanita
                            </span>
                          </p>
                        </div>
                ...
                <div className="card">
                  <Link to="/all-pengumuman">
                    <div className="card-body">
                      <div className="d-flex">
                        <div className="flex-grow-1">
                          <p className="text-truncate text-success font-size-18 mb-2">
                            Pengumuman
                          </p>
                          <h4 className="mb-2">
                            <label
                ...
                <div className="card">
                  <Link to="/all-pengajar">
                    <div className="card-body">
                      <div className="d-flex">
                        <div className="flex-grow-1">
                          <p className="text-truncate text-warning font-size-18 mb-2">
                            Guru
                          </p>
                          <h4 className="mb-2">
                            <label
                          ...
                          <p className="text-muted mb-0">
                            ...
                            >
                              <i className="ri-user-2-line me-1 align-middle"></i>
                              {jumlahPengajarPerempuan} Wanita
                            </span>
                          </p>
                        </div>
                        <div className="avatar-sm">
                ...
                <div className="card">
                  <Link to="/tanya-jawab">
                    <div className="card-body">
                      <div className="d-flex">
                        <div className="flex-grow-1">
                          <p className="text-truncate text-danger font-size-18 mb-2">
                            Tanya Jawab
                          </p>
                          <h4 className="mb-2">
                            <label
