...
function AllKelas() {
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          ...
          const defaultClass = data.find(
            (kelasItem) => kelasItem.nama_kelas === "X IPA 1"
          );
          if (defaultClass) {
  ...
  const handleEditFormSubmit = (event) => {
    ...
    fetch(`${BASE_URL}api/kelas/${editingKelas.id}`, {
      ...
      .then((data) => {
        ...
        setKelas(
          kelas.map((kelasItem) =>
            kelasItem.id === data.id ? data : kelasItem
          )
        );
  ...
  const studentPageCount =
    selectedKelas && selectedKelas.users
      ? Math.ceil(selectedKelas.users.length / STUDENTS_PER_PAGE)
      : 0;
  const studentOffset = currentStudentPage * STUDENTS_PER_PAGE;
  const currentStudentPageData =
    selectedKelas && selectedKelas.users
      ? selectedKelas.users.slice(
          studentOffset,
          studentOffset + STUDENTS_PER_PAGE
        )
      : [];
  ...
  const handleFormSubmit = (event) => {
    event.preventDefault();
    if (!namaKelas) {
      Swal.fire("Gagal!", "Field tidak boleh kosong!", "error");
      return;
    }
    const existingClass = kelas.find(
      (kelasItem) => kelasItem.nama_kelas === namaKelas
    );
    if (existingClass) {
      Swal.fire("Gagal!", "Kelas dengan nama ini sudah ada!", "error");
      return;
    }
              ...
              <div className="col-xl-3">
                <div className="card h-100">
                  <div className="card-body">
                    <button
                      type="button"
                      class="btn btn-info waves-effect waves-info w-100"
                      id="btn-new-event"
                        ...
                        kelas.map((kelasItem, index) => (
                          ...
                          >
                            ...
                            <button
                              ...
                              onClick={() =>
                                handleEditButtonClick(kelasItem.id)
                              }
                            ...
                            <button
                              ...
                              onClick={() =>
                                handleDeleteButtonClick(kelasItem.id)
                              }
                            >
                  ...
                  <div class="card-body">
                        ...
                        <tbody>
                          {selectedKelas ? (
                            ...
                            <tr>
                              <td colSpan="5">
                                <center>
                                  Pilih salah satu kelas untuk menampilkan
                                  daftar siswa
                                </center>
                              </td>
                            </tr>
                    ...
                    <ReactPaginate
                      ...
                      onPageChange={({ selected: selectedPage }) =>
                        setCurrentStudentPage(selectedPage)
                      }
                        ...
                        <div className="col-12">
                          <div className="mb-3">
                            ...
                            <input
                              ...
                              required=""
                              value={
                                editingKelas ? editingKelas.nama_kelas : ""
                              }
                              onChange={(event) =>
                                setEditingKelas({
                                  ...editingKelas,
                                  nama_kelas: event.target.value,
                                })
                              }
                            />
                          </div>
